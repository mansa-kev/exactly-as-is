...
export async function canManageLifecycle(
...
function parseInspectionBody(body: Record<string, unknown> | undefined): InspectionPayload {
  ...
  return {
    ...
    photo_fuel_mileage:
      typeof b.photo_fuel_mileage === 'string'
...
export function createBookingPickupHandler(supabase: SupabaseClient) {
  return async (req: Request, res: Response) => {
    ...
    try {
      ...
      const inspectionResult = await insertInspection(
        supabase,
        bookingId,
        'pre_handover',
        payload,
...
export function createBookingReturnHandler(supabase: SupabaseClient) {
  return async (req: Request, res: Response) => {
    const accessToken = getAccessToken(req);
    if (!accessToken) {
      return res.status(401).json({ success: false, error: 'Authorization header is required.' });
    ...
    try {
      ...
      const overtimeHours =
        body.overtime_hours != null ? Number(body.overtime_hours) : null;
      const overtimeCharge =
        body.overtime_charge != null ? Number(body.overtime_charge) : null;
