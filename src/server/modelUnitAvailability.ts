...
export async function isModelAvailableForDates(
  ...
  startDate: string,
  endDate: string,
  options: { ignoreReservationId?: string; ignoreBookingId?: string } = {}
): Promise<boolean> {
  ...
  if (candidateCarsError) return false;

  const candidateIds = (candidateCars || []).map((car: { id: string }) => car.id);
  ...
  if (blockingBookingsError) return false;
  ...
  if (blockingReservationsError) return false;
...
export async function getAvailableCarIdForModelDates(
  supabase: SupabaseClient,
  vehicleModelId: string,
  startDate: string,
  endDate: string,
  ...
  if (candidateCarsError) return null;
