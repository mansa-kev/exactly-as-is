 ...
 */
export async function processBookingPayoutSettlements(
  ...
  try {
    ...
    const meta = (bookingFull.metadata || {}) as Record<string, any>;
    ...
    if (ownerPayout > 0 && bookingFull.car_id) {
      ...
      if (!existingSupplierSettle) {
        await supabase.from('payout_settlements').insert({
          ...
          amount: ownerPayout,
          status: 'pending',
        }).then(null, (err: any) => console.error(`${logPrefix} Supplier settlement insert error:`, err));
      }
    }
    ...
    if (broker?.broker_id && Number(broker.broker_commission_amount) > 0) {
      ...
      if (!existingBrokerSettle) {
        await supabase.from('payout_settlements').insert({
          ...
          status: 'pending',
        }).then(null, (err: any) => console.error(`${logPrefix} Broker settlement insert error:`, err));
      }
    }
    ...
    if (outsource?.is_outsourced && outsource?.owner_email) {
      ...
      }).then(null, (err: any) => console.error(`${logPrefix} Owner email error:`, err));
    }
  } catch (err) {
