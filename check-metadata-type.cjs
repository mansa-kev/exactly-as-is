const { createClient } = require('@supabase/supabase-js');

const supabaseUrl = 'https://edroffvtzrowpsooszqh.supabase.co';
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVkcm9mZnZ0enJvd3Bzb29zenFoIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3NTc0MzY2MiwiZXhwIjoyMDkxMzE5NjYyfQ.jL_YxElgo1_RMcrsslERsioazsCTlwAzw-G0JylGJh8';

const supabaseAdmin = createClient(supabaseUrl, supabaseKey);

async function checkMetadataType() {
  // If we try to filter using JSON operators, it will fail if it's text.
  const { data, error } = await supabaseAdmin.from('bookings').select('id').eq('metadata->>test', '1').limit(1);
  if (error) {
    console.error("Error with JSON operator:", error);
  } else {
    console.log("JSON operator works! metadata is JSON/JSONB.");
  }
}

checkMetadataType();
