...
async function checkMetadata() {
  const { data, error } = await supabaseAdmin.from('bookings').select('metadata').limit(1);
  if (error) {
    console.error("Error querying bookings:", error);
  } else {
    console.log("metadata type:", typeof data[0]?.metadata);
    console.log("metadata value:", data[0]?.metadata);
  }
}
