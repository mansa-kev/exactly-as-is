...
async function canSaveContract(supabase, userId, bookingId) {
...
async function guestTokenAllowsSave(supabase, bookingId, statusToken) {
    if (!statusToken)
        return false;
    const { data: booking } = await supabase
        .from('bookings')
...
export function createContractSaveHandler(supabase, requireServiceRole = true) {
    return async (req, res) => {
        const accessToken = getAccessToken(req);
        try {
            const { bookingId, signatureData, contractData, contractPdfBase64, statusToken, } = req.body || {};
