...
function hasServiceRoleKey() {
    return Boolean(process.env.SB_SERVICE_ROLE_KEY ||
        process.env.SUPABASE_SERVICE_ROLE_KEY ||
        process.env.VITE_SUPABASE_SERVICE_ROLE_KEY);
}
export function createBookingDocumentUploadHandler(supabase, requireServiceRole = true) {
    return async (req, res) => {
        try {
            if (requireServiceRole && !hasServiceRoleKey()) {
                return res.status(500).json({
                    success: false,
                    error: 'Server misconfiguration: SUPABASE_SERVICE_ROLE_KEY is required for booking document uploads. Run scripts/fix_booking_document_upload_storage.sql if the bucket policy is missing.',
                });
            }
            ...
            const mime = typeof contentType === 'string' && contentType.length > 0
                ? contentType
                : 'image/jpeg';
