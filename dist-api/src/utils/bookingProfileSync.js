 ...
 */
export const BOOKING_DOC_TO_PROFILE_COLUMN = {
...
export function buildProfileSyncFromBooking(booking, bookingData) {
    ...
    for (const [docKey, column] of Object.entries(BOOKING_DOC_TO_PROFILE_COLUMN)) {
        ...
        if (url)
            updates[column] = String(url);
    }
    return updates;
}
/** Resolve client id from booking.client_id or guest email on user_profiles. */
export async function resolveClientIdForBooking(supabase, booking) {
...
export async function linkBookingAndSyncProfile(supabase, booking, bookingData) {
    if (!booking?.id)
        return { clientId: null, synced: false };
...
export async function applyProfileSyncFromBooking(supabase, clientId, booking, bookingData) {
    if (!clientId || !booking)
        return false;
